...
uint32 ChallengeModes::getItemRewardAmount(ChallengeModeSettings setting) const
{
    switch (setting)
    {
        ...
        case SETTING_SELF_FOUND:
            return selfFoundItemRewardAmount;
        case HARDCORE_DEAD:
            break;
    }
    return 0;
...
const std::unordered_map<uint8, uint32> *ChallengeModes::getAchievementMapForChallenge(ChallengeModeSettings setting) const
{
    switch (setting)
    {
        ...
        case SETTING_SELF_FOUND:
            return &selfFoundAchievementReward;
        case HARDCORE_DEAD:
            break;
    }
    return {};
...
private:
    ...
    static void LoadConfig()
    {
        ...
        if (sChallengeModes->enabled())
        {
            ...
            sChallengeModes->selfFoundEnable         = sConfigMgr->GetOption<bool>("SelfFound.Enable", true);
            ...
            sChallengeModes->selfFoundDisableLevel         = sConfigMgr->GetOption<uint32>("SelfFound.DisableLevel", 0);
            ...
            sChallengeModes->selfFoundXpBonus        = sConfigMgr->GetOption<float>("SelfFound.XPMultiplier", 1.0f);
            ...
            sChallengeModes->selfFoundItemRewardAmount        = sConfigMgr->GetOption<uint32>("SelfFound.ItemRewardAmount", 1);
            ...
            LoadStringToMap(sChallengeModes->selfFoundAchievementReward, sConfigMgr->GetOption<std::string>("SelfFound.AchievementReward", ""));
        }
    }
};
...
public:
    ...
    void OnPlayerLogin(Player* player) override
    {
        ...
        player->KillPlayer();
        player->GetSession()->KickPlayer(std::string("Your Hardcore character has died."));
    }
...
// Local addition (not upstream): blocks player-to-player trade and mail so
// the character's gear/consumables/gold can only ever come from what it
// finds or crafts itself.
class ChallengeMode_SelfFound : public ChallengeMode
{
public:
    ChallengeMode_SelfFound() : ChallengeMode("ChallengeMode_SelfFound", SETTING_SELF_FOUND) {}
    ...
    bool OnPlayerCanInitTrade(Player* player, Player* target) override
    {
        if (!sChallengeModes->challengeEnabledForPlayer(SETTING_SELF_FOUND, player) &&
            !sChallengeModes->challengeEnabledForPlayer(SETTING_SELF_FOUND, target))
        {
            return true;
        }
        return false;
...
// Local addition (not upstream): closes off the guild bank for Self-Found
// characters. Money is a hard block (the hook's amount is used for the real
// transfer afterwards); items can only be blocked at the UI level, since
// OnItemMove fires after the swap already committed and there's no hook to
// veto it. Matches the rest of the mod's honor-system threat model.
class ChallengeMode_SelfFound_GuildBank : public GuildScript
{
public:
    ChallengeMode_SelfFound_GuildBank() : GuildScript("ChallengeMode_SelfFound_GuildBank") {}
...
// challenge a character is running. IDs 178-186 are custom CharTitles.dbc rows; see
// sql/migrations/world/0034_up_challenge_modes_start_titles.sql for the server-side
// half and the module README for the required client DBC patch.
enum ChallengeModeStartTitles
{
    TITLE_HARDCORE           = 178,
    TITLE_SEMI_HARDCORE      = 179,
    TITLE_SELF_CRAFTED       = 180,
    TITLE_ITEM_QUALITY_LEVEL = 181,
    TITLE_SLOW_XP_GAIN       = 182,
    TITLE_VERY_SLOW_XP_GAIN  = 183,
    TITLE_QUEST_XP_ONLY      = 184,
    TITLE_IRON_MAN           = 185,
    TITLE_SELF_FOUND         = 186
};

static uint32 GetStartTitleForSetting(ChallengeModeSettings setting)
{
    switch (setting)
    {
        case SETTING_HARDCORE:           return TITLE_HARDCORE;
        case SETTING_SEMI_HARDCORE:      return TITLE_SEMI_HARDCORE;
        case SETTING_SELF_CRAFTED:       return TITLE_SELF_CRAFTED;
        case SETTING_ITEM_QUALITY_LEVEL: return TITLE_ITEM_QUALITY_LEVEL;
        case SETTING_SLOW_XP_GAIN:       return TITLE_SLOW_XP_GAIN;
        case SETTING_VERY_SLOW_XP_GAIN:  return TITLE_VERY_SLOW_XP_GAIN;
        case SETTING_QUEST_XP_ONLY:      return TITLE_QUEST_XP_ONLY;
        case SETTING_IRON_MAN:           return TITLE_IRON_MAN;
        case SETTING_SELF_FOUND:         return TITLE_SELF_FOUND;
