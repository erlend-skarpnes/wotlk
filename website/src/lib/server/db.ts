...
export interface ServerStatus {
	worldOnline: boolean;
	authOnline: boolean;
	uptimeSeconds: number | null; // null if worldserver offline
}
...
/** Live server status: TCP checks via Tailscale + uptime duration from DB. */
...
export async function getOnlineCharacters(): Promise<Character[]> {
	const [rows] = await pool().query(`
		SELECT c.name, c.level, c.race, c.gender, c.class, c.zone, 1 AS online
